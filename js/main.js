...
//jquery ui functions needed for slide effect
...
$slides = $('.theSlider .slide');
$numberOfSlides = $slides.length;
//create dots navigation
for (i = 0; i < $numberOfSlides; i += 1) {
	$('.sliderControls .dots').append('<i class="fa fa-dot-circle-o" aria-hidden="true"></i>');
}
//get dots
$dots = $('.dots i');
$firstDot = $dots[0];
$firstSlide = $slides[0];
$lastSlide = $slides[$numberOfSlides - 1];

// mark hidden slides and dots
$('.slide').not($firstSlide).hide().addClass('hideSlide');
$dots.not($firstDot).addClass('fadeDot');

//mark first slide as current
$('.theSlider').find($firstSlide).addClass('currentSlide');
$('.dots').find($firstDot).addClass('currentDot');

function getValues() {
	$currentNumber = $('.currentSlide').index();
	$nextSlideNumber = $('.currentSlide').index() + 1;
	$prevSlideNumber = $('.currentSlide').index() -1;

	if ($nextSlideNumber === $numberOfSlides) {
		$nextSlideNumber = 0;
	} else if ($prevSlideNumber === -1) {
		$prevSlideNumber = $numberOfSlides - 1;
	}

	//next dot and slide
	$nextSlide = $slides[$nextSlideNumber];
	$nextDot = $dots[$nextSlideNumber];
	$prevSlide = $slides[$prevSlideNumber];
	$prevDot = $dots[$prevSlideNumber];
